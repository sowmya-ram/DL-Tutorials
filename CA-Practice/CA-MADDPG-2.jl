...
OUNoise(n::Int) = OUNoise(0.15f0, 0.15f0, zeros(Float32, n))
...
function sample_minibatch(buffer::ReplayBuffer, batch_size::Int)
    ...
    return (
        ...
        [batch_actions[i] for i in idxs],
        [batch_rewards[i] for i in idxs],
        [batch_next_states[i] for i in idxs]
    )
end
...
function create_critic(state_dim::Int, action_dim::Int, n_agents::Int)
    ...
    Chain(
        Dense(state_dim + total_action_dim, 64, relu),
        Dense(64, 64, relu),
        Dense(64, 1)
    )  # ~5,185 to 6,625 parameters
end
...
function MADDPG(state_dim::Int, action_dim::Int, n_agents::Int; 
                ...
                lr_actor=0.0005f0, lr_critic=0.0005f0,  # Adjusted learning rates
...
function get_actions(maddpg::MADDPG, states, exploration_noise=true)
    ...
    for (i, agent) in enumerate(maddpg.agents)
        ...
        if exploration_noise
            action = agent.noise(action)  # Use OU noise
        end
...
# Update function with gradient clipping and loss logging
function update!(maddpg::MADDPG, states, actions, rewards, next_states)
    ...
    for (i, agent) in enumerate(maddpg.agents)
        ...
        # Debug critic input shapes
        critic_input = vcat(hcat(batch_states...), all_actions)
        println("Agent $i: Critic input shape = $(size(critic_input))")
        
        next_q = agent.target_critic(vcat(hcat(batch_next_states...), all_next_actions))
        target_q = batch_rewards[i] .+ agent.γ * next_q
        
        critic_loss = mean((agent.critic(critic_input) .- target_q).^2)
        println("Agent $i: Critic Loss = $critic_loss")
        
        grads = gradient(() -> critic_loss, params(agent.critic))
        for p in params(agent.critic)
            if !isnothing(grads[p])
                grads[p] = clamp.(grads[p], -1.0f0, 1.0f0)
            end
        end
        Flux.update!(agent.critic_optimizer, params(agent.critic), grads)
        ...
        println("Agent $i: Actor Loss = $actor_loss")
        
        grads = gradient(() -> actor_loss, params(agent.actor))
        for p in params(agent.actor)
            if !isnothing(grads[p])
                grads[p] = clamp.(grads[p], -1.0f0, 1.0f0)
            end
        end
        Flux.update!(agent.actor_optimizer, params(agent.actor), grads)
...
# Cooperative communication environment with adjusted rewards and action scaling
...
function step!(env::CoopCommEnvironment, actions)
    for i in 1:env.n_agents
        env.agent_positions[i] .+= actions[i] * 0.2  # Increased action scaling
        env.agent_positions[i] = clamp.(env.agent_positions[i], 0.0f0, 10.0f0)
    end
    rewards = [-norm(env.agent_positions[i] - env.landmarks[env.target])/2 for i in 1:env.n_agents]  # Scaled distance
    for i in 1:env.n_agents
        if norm(env.agent_positions[i] - env.landmarks[env.target]) >= 0.5
            rewards[i] -= 0.1f0  # Penalty for not reaching target
        end
    end
