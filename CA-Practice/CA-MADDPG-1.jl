...
function MADDPG(state_dim::Int, action_dim::Int, n_agents::Int; 
                ...
                lr_actor=0.0001f0, lr_critic=0.0001f0,  # Reduced learning rates
...
# Get actions with increased exploration noise
function get_actions(maddpg::MADDPG, states, exploration_noise=true)
    ...
    for (i, agent) in enumerate(maddpg.agents)
        ...
        if exploration_noise
            action = action .+ randn(Float32, size(action)) * 0.2f0  # Increased noise
        end
...
# Update function with gradient clipping
    ...
    for (i, agent) in enumerate(maddpg.agents)
        ...
        for p in params(agent.critic)
            if !isnothing(grads[p])
                grads[p] = clamp.(grads[p], -1.0f0, 1.0f0)  # Gradient clipping
            end
        end
...
# Cooperative communication environment with normalized states and enhanced rewards
...
function step!(env::CoopCommEnvironment, actions)
    ...
    # Enhanced rewards: -distance + bonus for reaching target
    ...
    for i in 1:env.n_agents
        if norm(env.agent_positions[i] - env.landmarks[env.target]) < 0.5
            rewards[i] += 10.0f0  # Bonus for reaching target
        end
    end
    # Normalize states to [-1, 1]
...
function train!(maddpg::MADDPG, env::CoopCommEnvironment, max_episodes::Int, max_steps::Int)
    ...
    for episode in 1:max_episodes
        ...
        if episode % 1000 == 0  # Log less frequently for 25000 episodes
...
function main()
    ...
    state_dim = 6
    action_dim = 2
    max_episodes = 25000  # Match your setup
    max_steps = 25
    n_eval_episodes = 100
