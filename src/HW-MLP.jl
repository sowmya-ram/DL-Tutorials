...
lenet = Flux.Chain(
	Flux.Conv((5, 5), 1 => 6, relu), #all filter sizes, number of layers are all also Hyperparameters
	Flux.MaxPool((2, 2)),
	Flux.Conv((5, 5), 6 => 16, relu),
	Flux.MaxPool((2, 2)),
	Flux.flatten,
	Flux.Dense(256 => 120, relu), #256 here is decided by the original image size, and then the convolutional part, how many layers, what filter sizes, Poolings etc.
...
Flux.Conv((5, 5), 1 => 6).weight # 5×5×1×6 Array{Float32, 4}
